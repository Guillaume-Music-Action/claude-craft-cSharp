...
```dart
// ❌ BAD : Logique métier dans le widget
...
// ✅ GOOD : Utilisation de BLoC/Repository
class UserProfilePage extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
...
```dart
// ❌ BAD : Nommage, pas de const
class userCard extends StatelessWidget {
  final String UserName;
  userCard(this.UserName);
}

// ✅ GOOD : Conventions respectées
class UserCard extends StatelessWidget {
...
```dart
// ❌ BAD : Pas de const, création à chaque build
Widget build(BuildContext context) {
  return Container(
    child: Text('Hello'),
  );
...
// ✅ GOOD : État immutable avec Equatable
...
// ✅ GOOD : Variable d'environnement
class ApiConfig {
  static const apiKey = String.fromEnvironment('API_KEY');
}

// ❌ BAD : Token en SharedPreferences
prefs.setString('auth_token', token);

// ✅ GOOD : Token en SecureStorage
...
```dart
// ❌ BAD : Pas de mock, dépendance réelle
