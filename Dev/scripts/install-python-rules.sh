...
# Rules supprimees (maintenant dans Common/): 01, 04, 05, 09, 10, 11
# Note: Python n'a pas de 08-quality-tools ni 11-security specifiques
TECH_RULES=(
    "02-architecture.md"
...
Description:
    9 fichiers de regles couvrant :
    - Clean Architecture / Hexagonal
    - Standards PEP 8, type hints
    - Tests pytest, coverage
    - Qualite (ruff, mypy, black)
...
copy_common_rules() {
    ...
    if [ "$dry_run" = "false" ] && [ $count -gt 0 ]; then
        log_success "$count Python-specific rules copied"
    fi
}
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --version) echo "install-python-rules.sh version ${VERSION}"; exit 0 ;;
